  ...
  return (
    <div className="absolute top-4 left-4 right-4 z-[60] max-w-md mx-auto flex gap-2 transition-all">
      <div className="flex flex-1 items-center bg-white rounded-full shadow-md border border-gray-200 px-5 py-3">
        ...
        />
        {/* Toggle between Search icon and Clear icon based on input */}
        {query.length > 0 ? (
          <button onClick={() => onQueryChange('')} className="text-gray-400 hover:text-gray-600 ml-3">
            <X size={20} />
          </button>
        ) : (
      ...
      {/* Cancel button escapes the search panel */}
      {uiState === 'SEARCHING' && (
        <button 
          onClick={onCancel}
          className="bg-white rounded-full px-4 text-sm font-semibold text-gray-700 shadow-md border border-gray-200 hover:bg-gray-50 transition-colors"
        >
          Cancel
        </button>
      )}
